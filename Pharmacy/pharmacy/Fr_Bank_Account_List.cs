...
namespace pharmacy
{
    public partial class Fr_Bank_Account_List : Form
    {
        public Fr_Bank_Account_List()
        {
            InitializeComponent();
        ...
        DataSet ds;
        string org_qu = "SELECT * FROM Tbl_Bank_Account";
        ...
        private void Fr_Bank_Account_List_Load(object sender, EventArgs e)
        {
            Load_Data();
        ...
        {
            if (Check_Input(out bool name, out bool sh))
                return;
            ...
            if (sh)
            {
                sql_search += " SH LIKE '%'+@s+'%'";
                cmd.Parameters.AddWithValue("@s", txt_SH.Text);
            }
        ...
        bool Check_Input(out bool name, out bool sh)
        {
            name = sh = false;
            if (IsNull(txt_Name.Text) && IsNull(txt_SH.Text))
            {
                Load_Data();
                return true;
            }
            name = !IsNull(txt_Name.Text);
            sh = !IsNull(txt_SH.Text);
            return false;
        }
        ...
        void SetHeader()
        {
            ...
            dgv1.Columns["SH"].HeaderText = "شماره حساب";
            dgv1.Columns["Name"].HeaderText = "نام حساب";
