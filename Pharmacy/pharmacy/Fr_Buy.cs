...
namespace pharmacy
{
    public partial class Fr_Buy : Form
    {
        public Fr_Buy()
        {
            InitializeComponent();
        ...
        DataSet ds;
        string org_qu_med = "Select id, Name, Tedad, Price_Buy, Tedad FROM Tbl_Medicine";
        bool cancel = false;
        ...
        private void cmb_Med_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmb_Med.SelectedIndex != -1)
            {
                ...
                txt_Med_Price.Text = (int.Parse(current["Price_Buy"].ToString()) * txt_Med_Num.Value).ToString();
            }
        }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            if (Check_Input())
                return;
            btn_Calc_Click(null, null);
            var sql_find_factor = "SELECT COUNT(code) FROM Tbl_Factor WHERE Code=@0";
            ...
            var sql_insert = "INSERT INTO Tbl_Factor (Code, Date, Cus_Name, Cus_Phone, Cus_Address, id_Med, Tedad, Price_Med, Price_Kh, Price_Takh, Price, Description)" +
        ...
        {
            var fr_Buy_List = new Fr_Buy_List();
            fr_Buy_List.ShowDialog();
        }
        ...
        private void btn_Buy_Check_Click(object sender, EventArgs e)
        {
            ...
            fr_Ch_P.txt_Desc.Text = "پرداخت فاکتور خرید به صورت چک به شماره فاکتور " + txt_Code.Text;
            fr_Ch_P.ShowDialog();
            ClearText();
        }
        ...
        private void btn_Buy_Nagh_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if ((int)balance < int.Parse(txt_Total.Text))
                {
                    MessageBox.Show("مبلغ خرید بیش از موجودی حساب است");
                    return;
                }
            }
            catch (Exception ex)
        ...
        private void txt_Med_Num_ValueChanged(object sender, EventArgs e)
        {
            ...
            {
                var current = ds.Tables["med"].Rows[cmb_Med.SelectedIndex];
                txt_Med_Price.Text = (Convert.ToInt32(current["Price_Buy"]) * txt_Med_Num.Value).ToString();
            }
        }
