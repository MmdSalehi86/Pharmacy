...
namespace pharmacy
{
    public partial class Fr_Personnel_List : Form
    {
        public Fr_Personnel_List()
        {
            InitializeComponent();
        ...
        DataSet ds;
        string org_qu = "SELECT * FROM Tbl_Personnel";
        ...
        {
            if (Check_Input(out bool name, out bool phone, out bool date))
                return;
            ...
            if (phone)
            {
                sql_search += " Phone LIKE '%'+@ph+'%' AND";
                cmd.Parameters.AddWithValue("@ph", txt_Phone.Text);
            }
            if (date)
            {
                sql_search += " Date_Recr BETWEEN @d1 AND @d2";
        ...
        bool Check_Input(out bool name, out bool phone, out bool date)
        {
            name = phone = date = false;
            if (IsNull(txt_Name.Text) && IsNull(txt_Phone.Text) && txt_Date1.Text.IndexOf(" ") != -1 && txt_Date2.Text.IndexOf(" ") != -1)
            {
                Load_Data();
                return true;
            }
        ...
        void SetHeader()
        {
            ...
            dgv1.Columns["Status"].HeaderText = "سمت";
            dgv1.Columns["Date_Recr"].HeaderText = "تاریخ استخدام";
            dgv1.Columns["Salary"].HeaderText = "حقوق";
