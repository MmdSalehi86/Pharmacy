...
namespace pharmacy
{
    public partial class Fr_Check_List : Form
    {
        public Fr_Check_List()
        {
            InitializeComponent();
        ...
        DataSet ds;
        string org_qu = "SELECT * FROM Tbl_Check_D";
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            ...
            if (date)
            {
                sql_search += " Date2 BETWEEN @d1 AND @d2 AND";
        ...
        private void btn_Vosool_Click(object sender, EventArgs e)
        {
            ...
            if (res == DialogResult.No)
                return;
            var sql_find_sh = "SELECT COUNT(id) FROM SH WHERE SH=@0";
            Db.SetConnection(Sql_Commands.Select);
            ...
            try
            {
                if ((int)Db.Adap.SelectCommand.ExecuteScalar() == 0)
                {
            ...
            sql_update = "UPDATE Tbl_Check_D SET Status=@0";
            Db.Adap.UpdateCommand.CommandText = sql_update;
